...
def test_goal_creation(test_db_session):
    ...
    goal = Goal(
        name="Emergency Fund", target_amount=Decimal("10000.00"), target_date=date(2025, 12, 31)
    )
...
def test_goal_large_amounts(test_db_session):
    ...
    goal = Goal(
        name="Large Goal", target_amount=Decimal("9999999999999.99"), target_date=date(2030, 12, 31)
    )
    test_db_session.add(goal)
    test_db_session.commit()
...
def test_goal_multiple_goals(test_db_session):
    ...
    goal1 = Goal(name="Goal 1", target_amount=Decimal("5000.00"), target_date=date(2025, 6, 30))
    goal2 = Goal(name="Goal 2", target_amount=Decimal("10000.00"), target_date=date(2026, 12, 31))
...
def test_goal_deletion(test_db_session):
    ...
    goal = Goal(name="To Delete", target_amount=Decimal("1000.00"), target_date=date(2025, 12, 31))
    test_db_session.add(goal)
    test_db_session.commit()
...
def test_goal_name_length(test_db_session):
    ...
    goal = Goal(name="A" * 255, target_amount=Decimal("1000.00"), target_date=date(2025, 12, 31))
    test_db_session.add(goal)
    test_db_session.commit()
