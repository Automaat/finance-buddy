...
```typescript
import { pgTable, serial, text, numeric, timestamp, date, boolean, integer } from 'drizzle-orm/pg-core';
...
export const snapshotValues = pgTable('snapshot_values', {
  id: serial('id').primaryKey(),
  snapshotId: integer('snapshot_id').references(() => snapshots.id, { onDelete: 'cascade' }).notNull(),
...
<script lang="ts">
  ...
  const change = calculateChange(data.currentNetWorth, data.currentNetWorth - data.changeVsLastMonth);
</script>
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    <Card>
      ...
      <CardContent>
        ...
        <p class="text-sm mt-2" class:text-green-600={data.changeVsLastMonth >= 0} class:text-red-600={data.changeVsLastMonth < 0}>
...
export async function load() {
  ...
  if (latestSnapshot.length > 0) {
    ...
    latestValues = Object.fromEntries(
      values.map(v => [v.accountId, v.value])
    );
  }
...
<div class="max-w-4xl mx-auto space-y-8">
  ...
  <form on:submit|preventDefault={handleSubmit} class="space-y-6">
    ...
    <div class="flex gap-4">
      ...
      <Button type="button" variant="outline" on:click={() => goto('/')}>
        Anuluj
      </Button>
    </div>
  </form>
...
<div class="space-y-8">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    {#each data.goals as goal}
      {@const progress = calculateGoalProgress(Number(goal.currentAmount), Number(goal.targetAmount))}
