...
test.describe('snapshots', () => {
	test('list page shows headers and create button', async ({ page }) => {
		...
		await expect(page.getByRole('heading', { name: 'Migawki', exact: true })).toBeVisible();
		await expect(page.getByRole('button', { name: /Nowa migawka/ })).toBeVisible();
	});

	test('new snapshot form renders with prefilled date and submit button', async ({ page }) => {
		await page.goto('/snapshots/new');
		await expect(page.getByRole('heading', { name: 'Nowa migawka' })).toBeVisible();
		...
		await expect(page.getByRole('button', { name: /Zapisz migawkę/ })).toBeVisible();
	});
